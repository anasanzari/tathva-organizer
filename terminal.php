...
<?php echo $session->getUsertype(); ?>
        ...
        <div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane" id="users">
                <div id="users">
                    ...
                    <thead>
                      <tr><th>ROLLNUMBER</th><th>Username</th> 
                        <th>Password</th><th>Eventcode</th> 
                        <th>Validation</th><th>Delete</th> 
                      </tr>
                    </thead>
                    <tbody>
                        <?php 
                        ...
                        foreach ($users as $user) {
                            $val = ($user->getValidate() == 0) ? "validate" : "invalidate";
                            $det = "<tr><td>".$user->getRoll()."</td>".
                                    "<td>".$user->getName()."</td>".
                                    "<td>".$user->getPassword()."</td>".
                                    "<td>".$user->getEventcode()."</td>".
                                    '<td><a class="u-validate-btn btn btn-default btn-xs" data-name="'.$user->getName().'">'
                                         .$val.'</a></td>'
                                    .'<td><a class="u-delete-btn btn btn-danger btn-xs" data-name="'.$user->getName().'">delete</a></td></tr>';
                            echo $det;
                    ...
                    <table class="table table-condensed">
                        ...
                        foreach ($eventDetails as $row) {
                            ...
                            $det = "<tr><td>".$row['code']."</td>".
                                    "<td>".$row['name']."</td>".
                                    "<td>".$row['cat']."</td>".
                                    '<td><a class="e-validate-btn btn btn-default btn-xs" data-name="'.$row['code'].'">'
                                         .$val.'</a></td>'
                                    .'<td><a class="e-delete-btn btn btn-danger btn-xs" data-name="'.$row['code'].'">delete</a></td>'
    ...
    <script>
        ...
        $(".u-validate-btn").click(function(){
            showProgress();
            var ele = $(this);
            var uname = $(this).attr("data-name");
            var val = $(this).text();
            $.getJSON('./get.php',{type:'usermanipulation',username:uname,userop:val},function(data){
                hideProgress();
                //console.log(JSON.stringify(data));
                if(data.status=='success'){
                    if(val=='validate') ele.text("invalidate");
                    else ele.text('validate');
                }else{
                    alert("this shouldnt be happening!! some kind of error occured");
                }
            });
        ...
        $(".u-delete-btn").click(function(){
            showProgress();
            var ele = $(this);
            var uname = $(this).attr("data-name");
            $.getJSON('./get.php',{type:'usermanipulation',username:uname,userop:'delete'},function(data){
                hideProgress();
                console.log(JSON.stringify(data));
                if(data.status=='success'){
        ...
        $(".e-validate-btn").click(function(){
            showProgress();
            var ele = $(this);
            var uname = $(this).attr("data-name");
            var val = $(this).text();
            $.getJSON('./get.php',{type:'eventmanipulation',eventcode:uname,eventop:val},function(data){
                hideProgress();
                console.log(JSON.stringify(data));
                if(data.status=='success'){
                    if(val=='validate') ele.text("invalidate");
        ...
        $(".e-delete-btn").click(function(){
            showProgress();
            var ele = $(this);
            var uname = $(this).attr("data-name");
            $.getJSON('./get.php',{type:'eventmanipulation',eventcode:uname,eventop:'delete'},function(data){
                hideProgress();
                console.log(JSON.stringify(data));
                if(data.status=='success'){
                    ele.parent("tr").remove();
